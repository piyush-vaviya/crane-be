...
const creatsendToken = (user, statusCode, res) => {
  ...
  console.log('🚀 ~ creatsendToken ~ process.env.NODE_ENV', process.env.NODE_ENV)
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ $or: [{ email }, { name: email }] }).select('+password') // select expiclity password
